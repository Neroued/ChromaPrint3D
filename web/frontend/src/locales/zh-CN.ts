...
export default {
  ...
  vectorize: {
    ...
    settings: {
      ...
      pipelineMode: '算法',
      pipelineModeHint:
        'V1：边界图 + 剪切模型，适合线稿和简单图形。V2：层叠模型 + 深度排序，适合照片和复杂图像。',
      pipelineV1: 'V1（边界图）',
      pipelineV2: 'V2（层叠模型）',
      quickTip: '调整颜色数量和平滑度即可快速调参，如需精调可在高级设置中微调曲线拟合。',
      quickTipV2: '调整颜色数量即可快速调参，如需精调可在高级设置中微调曲线拟合和轮廓简化。',
    ...
    advanced: {
      ...
      slicCompactness: '紧凑度',
      slicCompactnessHint: 'SLIC 紧凑度参数。越低越贴合颜色边缘；越高形状越规整。建议 4~10。',
      ...
      enableSubpixelRefine: '亚像素精修',
      enableSubpixelRefineHint: '梯度引导的亚像素边界精修，可获得更平滑的边缘。',
      subpixelMaxDisplacement: '最大位移 (px)',
      subpixelMaxDisplacementHint: '亚像素精修时沿法线方向的最大偏移量（像素）。建议 0.3~1.0。',
