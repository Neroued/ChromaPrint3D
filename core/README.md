...
`RasterProc` 类负责将输入图像处理为匹配引擎所需的格式：
...
输出 `RasterProcResult`，包含 `rgb`、`lab`、`mask` 三个矩阵。
...
支持 `ProgressCallback` 按阶段报告进度（LoadingResources → Preprocessing → Matching → BuildingModel → Exporting）。
